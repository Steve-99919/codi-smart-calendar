...
const handleTestStatusReminders = async (req: Request): Promise<Response> => {
  try {
    ...
    // Get the test date from query parameters or use today's date
    ...
    // Calculate a "yesterday" date for the test
    let yesterday;
    if (testDate) {
      // If a test date is provided, use it as "yesterday"
      yesterday = new Date(testDate);
    } else {
      // Otherwise use actual yesterday
      yesterday = new Date(now);
      yesterday.setDate(yesterday.getDate() - 1);
    }
    
    const yesterdayFormatted = yesterday.toISOString().split('T')[0];
    console.log(`Using test prep date: ${yesterdayFormatted}`);
    
    // Get activities whose prep_date matches our test "yesterday" date
    ...
    console.log(`Found ${activities?.length || 0} activities with prep date: ${yesterdayFormatted}`);
    ...
    // Send reminders (or simulate if dryRun is true)
